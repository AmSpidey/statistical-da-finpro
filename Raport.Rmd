...
lasso_best <- glmnet(as.matrix(fullCancPred), as.matrix(fullCancRes), lambda = lassoCV$lambda.min)

lasso.canc.cv.err <- (lassoCV$cvm[lassoCV$lambda == lassoCV$lambda.min])

predTrain <- predict(lasso_best, fullCancPred)
...
ridge_best <- glmnet(as.matrix(fullCancPred), as.matrix(fullCancRes), lambda = ridgeCV$lambda.min)

ridge.canc.cv.err <- (ridgeCV$cvm[ridgeCV$lambda == ridgeCV$lambda.min])
predTrain <- predict(ridge_best, fullCancPred)
