...
lasso_non_zeroes <- function (lasso_best, yX, ytXt) {
  ...
  to_ret <- list ("subData" = subData, "subTest" = subTest)
  return(to_ret)
}
...
predTrain <- predict(stepaic, newdata = sub$subData)
MSEBICProt <- MSE(predTrain, res)
predTest <- predict(stepaic, newdata = sub$subTrain)
