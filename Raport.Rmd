...
predTrain <- predict(lasso_best, s = lassoCV$lamba.min, newx = as.matrix(fullProtPred))
lasso.prot.train.err <- MSE(predTrain, fullProtRes)

pred.protein <- predict(lasso_best, s = lassoCV$lamba.min, newx = as.matrix(data.test.prot))

```


```{r}
# choosing most important 5 predictors with leapForward. It searches for the best model of size 5 and that's the reason I chose it.
...
sum <- summary(reg)
predictors.protein <- as.data.frame(names(coef(reg,5))[-1]) # TABLE OF THE MOST IMPORTANT 5 PREDICTORS FOR PROTEIN
```
...
protCVs
```
I am choosing lasso, since it has better results. Leap forward seems to perform worse on train test, too. Lasso seems to have better generalization properties.
...
# I will explain later why I use ridge for the predictions.
pred.cancer <- predict(ridge_best, s = ridgeCV$lamba.min, newx = as.matrix(data.test.canc))
...
Finally I decide to use ridge because it seems to be having smaller test error relatively to lasso. The differences between test and train error seem less important here.
