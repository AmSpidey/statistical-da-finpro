...
# seems like the optimal number of predictors is 5 looking at RMSE statistics (I ran this around 10 times and got 6 parameters once). Let's choose 5. Below code will be using 5 even if this happened to return 6 as the optimal number! I did this that way for some consistency.
...
reg100cv <- ((reg100final$results)[5, "RMSE"])^2
predTrain <- predict(reg100final, fullProtPred)
reg100trainmse <- MSE(predTrain, fullProtRes)
```

```{r}
# In the end, I decide to go with the 5 predictors. From MSE we can see that lasso tends to overfit and so does stepAIC. Let's see the VIF histogram!
...
Finally I decide to use lasso because it seems to be having smaller difference between train and test error relatively to ridge. Plus, it has a pretty neat cv error.
