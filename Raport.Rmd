...
leap <- train(data, res, method = "leapForward", tuneGrid = expand.grid(nvmax = 5))
pred <- predict(leap, newx = as.matrix(testData))
...
```{r}
# stepBIC
steps <- train(data,res, method = "glmStepAIC")

empty.mod <- lm(Y ~ 1, data = subData)
all.mod <- lm(Y ~ ., data = subData)
stepaic <- stepAIC(initialModel,direction="forward",
    scope=list(lower=initialModel,
        upper=allModel)
    )

form <- formul(subData[-ncol(subData)])
is.data.frame(subData)
stepBic <- train(x = subData[,-ncol(subData)], y = subData[,ncol(subData)], method = "glmStepAIC", direction = "forward")
stepAic <- bic(subData)
stepAic
