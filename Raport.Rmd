...
lasso_non_zeroes <- function (lasso_best, yX, ytXt) {
  cs <- coef(lasso_best)
  nonzero <- (which(cs[,1]!=0) - 1)[-1]
  r <- rownames(as.data.frame(nonzero))
  subTrain <- cbind(yX[,nonzero], yX[ncol(yX)])
  subTest <- cbind(ytXt[,nonzero], ytXt[ncol(ytXt)])
  to_ret <- list ("subTrain" = subTrain, "subTest" = subTest)
  return(to_ret)
}
...
```{r}
sub <- lasso_non_zeroes(lasso_best, as.data.frame(train), as.data.frame(test))
