...
div <- sample(podzial, length(podzial))
test.errs <- numeric(10)
train.errs <- numeric(10)

for (i in 1:k) {
  train_indx <- which(podzial != i)
  test_indx <- which(podzial == i)
  
  empty.mod <- lm(Y ~ 1, data = cv_data)
  all.mod <- lm(Y ~ ., data = cv_data)
  model <- stepAIC(empty.mod, direction="forward",
      scope=list(lower=empty.mod,
          upper=all.mod)
      )
  ...
  test.err <- mean((pred - cv_data[test_indx, "V2"])**2)
