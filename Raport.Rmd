...
reg100 <- train(x = data, y = res, tuneGrid = expand.grid(nvmax = c(1:100)), method = "leapForward", trControl = trCtrl)
...
coef(reg100$finalModel, 5)
# Let's try out hold out method first.

predProt <- paste(predictors.protein, collapse = "+")
form <- as.formula(paste("Y ~", predProt))

lm5Prot <- lm(form, as.data.frame(train))
predTrain <- predict(lm5Prot, as.data.frame(train))
MSELM5ProteinTrain <- MSE(predTrain, res)
predTest <- predict(lm5Prot, as.data.frame(test))
MSELM5ProteinTest <- MSE(predTest, testRes)

library(lmvar)
# Cross-validation
lm.prot.cv <- cv.lm(lm(form, as.data.frame(data.train.prot), y = TRUE, x = TRUE))
lm.prot.cv.err <- lm.prot.cv$MSE$mean
```
